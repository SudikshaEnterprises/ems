...
class Thana extends CI_Controller {
    ...
    public function index ()
    {
        ...
        $data['content']    = $this -> load -> view ('constitutency/thana/listing', $data, true);
        $this -> load -> view ('templates/template', $data);
    }

    function validate_thana ()
    {
        ...
        {
            $this -> form_validation -> set_message ('validate_thana', 'Thana Name already exist for this year ');
            return false;
        }
        return true;
    }
    ...
    public function add ()
    {
        $this -> form_validation -> set_rules ('unique_id', 'Thana ID', 'trim|required|callback_validate_thana');
        ...
        {
            $data['party_data'] = $this -> common_model -> get ('party');
            $data['content']    = $this -> load -> view ('constitutency/thana/add', $data, true);
            $this -> load -> view ('templates/template', $data);
        }
        else
        {
            ...
            $this -> common_model -> insert ('thana', $content);

            set_message_admin ('Thana added successfully', 'success');
            redirect ('/constitutency/thana');
        }
    }
    ...
    public function csv_upload ()
    {
        ...
        $db_fields = ['year', 'unique_id', 'english_name', 'hindi_name'];
    ...
    function delete ($id)
    {
        ...
        {
            $where = ['id' => $id];
            $this -> common_model -> delete ('thana', $where);
    ...
    function edit ($id)
    {
        $this -> form_validation -> set_rules ('unique_id', 'Thana ID', 'trim|required|callback_validate_thana_edit');
        $this -> form_validation -> set_rules ('year', 'Year', 'trim|required');
        $this -> form_validation -> set_rules ('english_name', 'English Name', 'trim|required');
        ...
        {
            $where              = ['id' => $id];
            $data['thana']      = $this -> common_model -> get ('thana', $where);
            $data['party_data'] = $this -> common_model -> get ('party');

            $data['content'] = $this -> load -> view ('constitutency/thana/edit', $data, true);
            $this -> load -> view ('templates/template', $data);
        }
        else
        {
            ...
            $where = ['id' => $id];
            $this -> common_model -> update ('thana', $content, $where);
    ...
    function download_csv ()
    {
       ...
       // header ("Expires: 0"); // Proxies
        ...
        $thana = $this -> common_model -> get ('thana');
